        ...
        threads.push_back(std::thread([i, numRounds, numEnqueues, &queue, &dataGen]() {
            for (unsigned long no = 0; no < numRounds; ++no) {
                ...
                for (unsigned long n = 0; n < numEnqueues; ++n) {
                    ...
                    if (i % 2 == 0) {
                        ASSERT(queue.push(dataGen(i, n)) == QueueResult::Success);
                    }
                    else {
...
void testCloseAbort(QueueStopper&& queueStopper, Queue& queue, DataGen&& dataGen, unsigned int numEnqueues, unsigned int numThreads) {
    std::vector<std::thread> threads;
    ...
    for (unsigned int i = 0; i < numThreads; ++i) {
        threads.push_back(std::thread([stopRound, i, numEnqueues, &queueStopper, &queue, &dataGen]() {
            try {
                ...
                ASSERT(stop);  // We intend to stop inbetween
                if (i % 2 == 0) {
...
CASE("LowLatencyQueue<string, 256>() test abort") {
    using Queue = LowLatencyQueue<std::string, std::integral_constant<std::size_t, 256>>;
    Queue queue{};
    testCloseAbort<std::string>([](Queue&) {
        throw std::runtime_error("Test abort with runtime error");
    },
                                queue, createStringFunctor(), queue.capacity() * 100 / std::thread::hardware_concurrency(), std::thread::hardware_concurrency());
}

CASE("LowLatencyQueue<string, 256>() test close") {
    using Queue = LowLatencyQueue<std::string, std::integral_constant<std::size_t, 256>>;
    Queue queue{};
    testCloseAbort<std::string>([](Queue& q) {
        q.close();
    },
                                queue, createStringFunctor(), queue.capacity() * 100 / std::thread::hardware_concurrency(), std::thread::hardware_concurrency());
    ASSERT(queue.size() < 0);
