...
/**
 * This queue is a circular buffer with atomitc synchronization (no mutex, no kernel).
 * It offers an optimistic push/pop that is very fast due to creating tickets with fetch-and-add (FAA)
 * instructions and avoiding loops over compare-and-swap (CAS) operations.
 *
 * There are also pessimistic approaches tryPush/tryPop that can be used as well.
 * However, using the optimistic approach is encouraged.
 * Using both mechanisms is possible but the resulting behaviour is unknown. Under high-contention the
 * pessimistic push/pop are likely to always fail against optimistic ones.
 *
 * Spinning:
...
// Alternatively a sleep_for or sleep_until can be used or more complex wakeups through conditional-variables and mutexs
// can be used. However this would be a hybrid approach to switch between high-throughput and no-work phases.
// By Customizing a PushPopSpinFunctor, it is possible to introduce global conditional-varibales.
//
// Note: getting the with chrono is very fast on linux because virtual ELF dynamic shared objects (vdso) are used.
...
// Default Functior with methods that get called when an element has been pushed or popped.
// More over this functor is ment to create spin functors to allow sharing additional queue states.
// For example to wait for a global condition variable.
