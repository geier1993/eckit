...
constexpr inline IntType bitWidth(IntType v) noexcept {
    static_assert(std::is_integral_v<IntType> && !std::numeric_limits<IntType>::is_signed, "bitWidth is only valid for unsigned integral types");
    ...
    const IntType np2 = nextPowOf2(v);
    return bitWidth(v == np2 ? (np2 << 1) : np2, (IntType)0, std::integral_constant<std::size_t, sizeof(IntType) * CHAR_BIT>{});
};
