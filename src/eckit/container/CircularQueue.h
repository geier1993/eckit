...
#include "eckit/memory/AlignedAllocator.h"
...
struct SpinThread {
    constexpr void operator()(bool outOfPeriod) const noexcept {
        ...
        // For hyberthreaded threads, this can be hard to find. If a thread has enough work, 
...
// For applications that are expecting to have phases of no work, rescheduling should happen.
// Alternatively a sleep_for or sleep_until can be used or more complex wakeups through conditcional-variables and mutexs
// can be used. However this would be a hybrid approach to switch between high-throughput and no-work phases.
template<std::size_t MaxWaitBeforeYieldMuS = 100, bool RescheduleThreadOnOversubscription=false>
struct SpinTimeThread {
    ...
    void operator()(bool outOfPeriod) const noexcept {
        if(RescheduleThreadOnOversubscription && outOfPeriod) std::this_thread::yield();
        auto spinTime= std::chrono::steady_clock::now();
        if (std::chrono::duration_cast<std::chrono::microseconds>(spinTime - startSpin).count() > MaxWaitBeforeYieldMuS) {
            std::this_thread::yield();
        }
    }
};
...
namespace details {
/* Computes the number of bits that can be shuffled without exceeding a maximal representable value.
 */
 ...
 *
 * The user has to make sure that the max index is at least represented by a number of width `NumBits*2+space`.
 * Usuallaly the space is zero or might be computed from maxShuffleBitWidth(maxIndex)-2*NumBits
 */
...
constexpr inline IntType mapIndex(IntType i, std::size_t space=0) noexcept {
  ...
  //
  // This works because 
  //  * XOR is commutative & associative, e.g.: (H H H ^ L L L) ^ H H H = (H H H ^ H H H) ^ L L L = 0 0 0 ^ L L L
  //  * 0 is the identity/neutral element: 0 0 0 ^ L L L = L L L
  // 
  // For the higher ordered part and the space between, the initial information is preserved as
  // X ^ 0 = X
...
//   The effect of padding is very minimal in this case anyway... the loss due to cacheline alignment is much higher. 
//   Experimenting with a combined state array the case of a memory benefit actually never showed up.
template<typename T, std::size_t N, bool Align=true, typename INDT=unsigned int>
struct EntryPack {
    // static_assert(std::atomic<EntryState>::is_always_lock_free, "EntryState must be always lock free");
    ...
    struct DL: DLBase<U> {
        ...
        std::atomic<INDT> ticket; 
            // Will be manually initialized by container. Holds the head/tail index that's supposed be read next
        ...
        void reset() noexcept {
            ...
            // A clear state (unused or used, but not Acquiring or Releasing) needs to be assumed
    ...
    //  Non atomic - may not be used when this or other are accessed by multiple threads... but that should be clear
    void moveAssign(EntryPack&& other) noexcept(std::is_nothrow_move_constructible<T>::value) {
        for (std::size_t i = 0; i < N; ++i) {
            ...
            // Can only safely move Used entry states. This does not work if the other is actively used ofc...
...
struct EntryTraits {
    ...
    //   * For very large elemnets the cacheline invalidation is no big problem anyway. 
    ...
    static constexpr std::size_t EstimatedNumShuffleBits = TestMinNumShuffleBits + argmin(
        densityForAlignedPack<1<<TestMinNumShuffleBits, MaxMultipleOfCacheline>(), 
    ...
    static constexpr std::size_t MinNumShuffleBits = TestMinNumShuffleBits + argmin(
        densityForAlignedPack<1<<TestMinNumShuffleBits, 1>(), 
...
struct EntryContainer {
    ...
    EntryContainer(EntryContainer&& other) noexcept: shuffleSpace_{other.shuffleSpace_}, entries_{std::move(other.entries_)} {};
 ...
 * The size of the buffer should be larger than the size of producers and consumers. 
 * Otherwise oversubscription happens and multpile producers/consumers will content for the same entry.
 * Moreover chosing a power of two as size is always a good option to speed up some arithmetic computations.
 *
 * The buffer is caring about cache alignment and packs multiple entries with a value state to have some kind of 
 * trade-off between introducing memory padding for alignment and having a compact memory usage.
 * To avoid subsequent memory lookups an the same cacheline, the memory acceses on the buffer are shuffled to 
 * happen in a different order. To do this, the size of the buffer might chose a larger size than desired, 
 * but is newer smaller.
 *
 * In general, as long as the size of a single entry is not super big, the minimum size of the buffer might be around
 * 16 entries. 
 * Example sizes for 64 L1 cache:
 *  * Storing int (4Byte) + state -> Entry size of 8 bytes, 8 elements per cache line. 
 *    For fast shuffeled index access, subpackaging to powers of 2 is done. 
 *    Hence 2^3 = 8 values are packed together on 2 cache lines. 
 *    For shuffling the minimum size is then 2^6 = 64 entries, which is also optimal
 *  * However for 2 int (8Byte) + state -> Entry size of 12 bytes, 5 elemnts per cacheline
 *    For fast shuffeled index access, subpackaging to powers of 2 is done. 
 *      * Hence for a minimal container size 2^2 = 4 values are packed together on 1 cache lines. 
 *        For shuffling the minimum size is then 2^4 = 16 entries
 *      * For more condensed packing instead of 4 elements on 1 CL (16 Bytes per elem), 
 *        we can also pack 16 elements on 3 cachelines (12 Bytes per elem)
 *        This then results in a minimum container size of 256 elements
 *
 *    For static sized container the layout is already adjusted. However, for dynamically sized containers, 
 *    currently just the more dense packing is used.
 *
 *
 * Comment on FIFO ordering:
 *   It is hard to say whether operations on the buffer are guaranteed to happen in FIFO order. 
 *   Defining FIFO order is difficult when multiple threads are involved.
 *   A lot of atomic operations can be forced to happened in sequentially-consistent order 
 *   to avoid having the compiler performing reording between operations on different atomic variables.
 *
 *   However this is only important for the operations on as single thread. For an optimistic push/pop
 ...
 *   For 20 years a known problem with lock-free queues implemented through lists is a problem known as ABA problem.
 *   In short: 1 thread does a lot of things while for another thread nothing appears to have changed. 
 ...
 *   through good work-balancing. In some cases it's also desired to limit the size and block the whole pipeline.
 *
 */
template<typename T, class AllocatorOrSize, typename DefaultSpinFunctor_, typename INDT>
...
private:
    ...
    // head_ and tail_ are expected to occupy about roughly the half of the representable value range.
    // This is the limit where we apply modulo operation to reset it freuently to prevent overflows in long runs
    const INDT PERIOD;
    const INDT HALF_PERIOD;
    
    static constexpr INDT FORCE_RESET_PERIOD_EXCEED = 1<<10; // 1024
    
    // The period for resetting the head & tail must be a multiple of the container size
    // Such that for any head & tail index `i` the following statement is true:
    // i % containerSize == (i % PERIOD) % containerSize
...
public:
    CircularBufferBase(const CircularBufferBase&) = delete;
    ...
    CircularBufferBase(ContainerArgs&& ... containerArgs) 
        noexcept(noexcept(ContainerType{std::forward<ContainerArgs>(containerArgs)...})): 
            head_{0}, 
            tail_{0}, 
            abort_{false},
...
private:
    ...
    bool pushIndex(INDT ticket, SpinFunctor&& spin, ElemOrArgs&& ... elemOrArgs) 
            noexcept(
                noexcept(T(std::forward<ElemOrArgs>(elemOrArgs)...)) && 
                noexcept(spin(false))) {
        ...
        return container_.visit(ind, 
            [&](auto& entry){
                ...
                INDT currentTicket;
                while UNLIKELY((currentTicket = entry.ticket.load(std::memory_order_acquire)) != ticket) {
    ...
    bool popIndex(INDT ticket, MoveElemFunctor&& moveElem, SpinFunctor&& spin) 
        noexcept(
        ...
        return container_.visit(ind, 
            [&](auto& entry){
                ...
                INDT currentTicket; 
                while UNLIKELY((currentTicket = entry.ticket.load(std::memory_order_acquire)) != usedTicket) {
                    if(abort_.load(std::memory_order_relaxed)) {
                        return false;
                    }
    ...
    INDT nextIndOpt(std::atomic<INDT>& av, const UInt size) noexcept {
        ...
        INDT v = av.fetch_add(1, std::memory_order_relaxed); 
            // See comment on CircularBufferBase. 
            // We could use `memory_order_seq_cst` to avoid that other increments on the same thread
            // are reordered and disturb fifo order - however due to the following acquire/release semantics
            // on the state, this kind of reordering is disallowed (IMO ... I might be wrong).
        ...
        if UNLIKELY( nextExpectedValue >= PERIOD ) {
            ...
            do  {
                ...
                !av.compare_exchange_weak( nextExpectedValue, newExpectedValue, std::memory_order_relaxed ) && 
    ...
    long computeSize(INDT h, INDT t) noexcept {
        h = modPeriod(h);
        t = modPeriod(t);
        // We assume that the typical difference between head and tail is much smaller than at least a forth 
        // of the representable range. 
        // On both - head and tail - a modulo operation might be performed. This never happens at the same time.
        // It is possible that head has been reset and holds a low value while tail holds a high value and vice versa.
    ...
    // Pessimistic approach is checking sizes and allow failing if size requirements are not met.
    // Only use this if you want to do something else if the push/depush is failing. 
    // If you just want to control how to bypass the waiting/spinning,
    ...
    std::optional<INDT> nextHeadPes() noexcept {
        ...
        // If many other producers are enqueuing optimistically, this is likely to fail
        do  {
...
public:
     ...
     * @return             True if successful, false if has been aborted
     */
    template<typename SpinFunctor, typename ... ElemOrArgs>
    bool pushWithSpinFunctor(SpinFunctor&& spin, ElemOrArgs&& ... elemOrArgs) 
        noexcept(
            noexcept(T(std::forward<ElemOrArgs>(elemOrArgs)...)) &&
            noexcept(spin(false))) {
        ...
        return pushIndex(nextHeadOpt(), std::forward<SpinFunctor>(spin), std::forward<ElemOrArgs>(elemOrArgs)...);
    }
    
    /* Calls `pushWithSpinFunctor` with the default spin functor specified through template argument of the CircularQueue.
    ...
    bool push(ElemOrArgs&& ... elemOrArgs) noexcept(noexcept(pushWithSpinFunctor(DefaultSpinFunctor_{}, std::forward<ElemOrArgs>(elemOrArgs)...))) {
        return pushWithSpinFunctor(DefaultSpinFunctor_{}, std::forward<ElemOrArgs>(elemOrArgs)...);
    }
     ...
     */
    template<typename MoveElemFunctor, typename SpinFunctor>
    bool popWithSpinFunctor(MoveElemFunctor&& moveElem, SpinFunctor&& spin)  
        noexcept(
            noexcept(std::forward<MoveElemFunctor>(moveElem)(std::move(std::declval<T>()))) && 
            noexcept(spin(false))) {
        return popIndex(nextTailOpt(), std::forward<MoveElemFunctor>(moveElem), std::forward<SpinFunctor>(spin));
    }
    
    /* Calls `popWithSpinFunctor` with the default spin functor specified through template argument of the CircularQueue.
     * Default is a busy-wait.
     *
     * @param moveElem  Functor that gets called with an rvalue of the element to be poped. The user can decide what to do.
     * @return          True if successful, false if has been aborted
    ...
    void pop(MoveElemFunctor&& moveElem) 
        noexcept(
            noexcept(std::forward<MoveElemFunctor>(moveElem)(std::move(std::declval<T>()))) && 
            noexcept(DefaultSpinFunctor_{}(false))) {
        return popWithSpinFunctor(std::forward<MoveElemFunctor>(moveElem), DefaultSpinFunctor_{});
    }
    
    /* Calls `popWithSpinFunctor` with the default spin functor specified through template argument of the CircularQueue.
     * Default is a busy-wait.
     *
     * @return        Returns the poped value or nothing if the deque has been aborted.
     */
    std::optional<T> popAsOpt()  
        noexcept(
            noexcept(std::is_nothrow_move_constructible<T>::value) && 
            noexcept(DefaultSpinFunctor_{}(false))) {
        std::optional<T> ret;
        popWithSpinFunctor([&ret](T&& v) noexcept(noexcept(ret = std::move(v))) { 
                ret = std::move(v);  // Will call move contructor on transition from nothing to something
            }, 
            DefaultSpinFunctor_{}
        );
        return ret;
     ...
     */
    template<typename SpinFunctor, typename ... ElemOrArgs>
    bool tryPushWithSpinFunctor(SpinFunctor&& spin, ElemOrArgs&& ... elemOrArgs)  
        noexcept(
            noexcept(T(std::forward<ElemOrArgs>(elemOrArgs)...)) &&
            noexcept(spin(false))) {
    ...
    /* Calls `tryPushWithSpinFunctor` with the default spin functor specified through template argument of the CircularQueue.
     * Default is a busy-wait.
     *
     * @param elemOrArgs   Forwards all arguments to the constructor of the element type T. 
     *                     (May perform a copy or move construction or just emplaces a value).
     ...
     */
    template<typename ... ElemOrArgs>
    bool tryPush(ElemOrArgs&& ... elemOrArgs) noexcept(noexcept(tryPushWithSpinFunctor(DefaultSpinFunctor_{}, std::forward<ElemOrArgs>(elemOrArgs)...))) {
        return tryPushWithSpinFunctor(DefaultSpinFunctor_{}, std::forward<ElemOrArgs>(elemOrArgs)...);
    }
     ...
     *
     * Notes: 
     *   * When you end up having a loop just calling tryPop until it succeeds, it is better to use the
     *     optimistic `pop`.
     ...
     */
    template<typename MoveElemFunctor, typename SpinFunctor>
    bool tryPopWithSpinFunctor(MoveElemFunctor&& moveElem, SpinFunctor&& spin)  
        noexcept(
            noexcept(std::forward<MoveElemFunctor>(moveElem)(std::move(std::declval<T>()))) && 
            noexcept(spin(false))) {
        auto indt = nextTailPes();
        if(indt) {
            return popIndex(indt, std::forward<MoveElemFunctor>(moveElem), std::forward<SpinFunctor>(spin));
        }
        return false;
    }
    
    template<typename MoveElemFunctor, typename SpinFunctor>
    std::optional<T> tryPopWithSpinFunctor(MoveElemFunctor&& moveElem, SpinFunctor&& spin) noexcept {
        auto indt = nextTailPes();
        if(indt) {
            return std::optional<T>{popIndex(indt, std::forward<SpinFunctor>(spin))};
        }
        return std::optional<T>{};
    }
    
    /* Calls `tryPopWithSpinFunctor` with the default spin functor specified through template argument of the CircularQueue.
     * Default is a busy-wait.
     *
     * @param moveElem  Functor that gets called with an rvalue of the element to be poped. The user can decide what to do.
     * @return          Returns true if deque has been successful.
     */
    template<typename MoveElemFunctor>
    bool tryPop(MoveElemFunctor&& moveElem)  
        noexcept(
            noexcept(std::forward<MoveElemFunctor>(moveElem)(std::move(std::declval<T>()))) && 
            noexcept(DefaultSpinFunctor_{}(false))) {
        return tryPopWithSpinFunctor(std::forward<MoveElemFunctor>(moveElem), DefaultSpinFunctor_{});
    }
     ...
     */
    std::optional<T> tryPopAsOpt()  
        noexcept(
            noexcept(std::is_nothrow_move_constructible<T>::value) && 
            noexcept(DefaultSpinFunctor_{}(false))) {
        std::optional<T> ret;
        tryPopWithSpinFunctor([&ret](T&& v) noexcept(noexcept(ret = std::move(v))) { 
...
template<typename T, class AllocatorOrSize = AlignedAllocator<T,CACHELINE_SIZE_DESTRUCTIVE>, typename SpinFunctor=DefaultSpinFunctor, typename INDT=unsigned int>
class CircularQueue: public details::CircularBufferBase<T, AllocatorOrSize, SpinFunctor, INDT> {
    private:
        using Base = details::CircularBufferBase<T, AllocatorOrSize, SpinFunctor, INDT>;
    public:
        CircularQueue(const CircularQueue&) = delete;
        
        CircularQueue(CircularQueue&& other) noexcept: Base(std::move(other)) {
        };
        ...
        CircularQueue(std::size_t size) noexcept(noexcept(Base(size))): Base(size) {};
};

template<typename T, typename IntType, IntType N, typename SpinFunctor, typename INDT>
struct CircularQueue<T, std::integral_constant<IntType, N>, SpinFunctor, INDT>: public details::CircularBufferBase<T, std::integral_constant<IntType, N>, SpinFunctor, INDT> {
    private:
        using Base = details::CircularBufferBase<T, std::integral_constant<IntType, N>, SpinFunctor, INDT>;
    public:
        CircularQueue(const CircularQueue&) = delete;
        ...
        CircularQueue() noexcept(noexcept(Base())): Base() {};
};
